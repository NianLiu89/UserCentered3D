...
public class Loader {
    ...
    public RawModel loadToVAOWithRgb(float[] positions, float[] rgbCoords,
            int[] indices) {
        int vaoID = createVAO();
        bindIndicesBuffer(indices);
        storeDataInAttributeList(0, 3, positions);
        storeDataInAttributeList(1, 3, rgbCoords);
        unbindVAO();
        return new RawModel(vaoID, indices.length);
    }
    
    public RawModel loadToVAOWithTexture(float[] positions, float[] textureCoords,
            int[] indices) {
        int vaoID = createVAO();
        bindIndicesBuffer(indices);
        storeDataInAttributeList(0, 3, positions);
    ...
    public RawModel loadToVAOWithTextureWithNormal(float[] positions, float[] textureCoords, float[] normals,
            int[] indices) {
        int vaoID = createVAO();
        bindIndicesBuffer(indices);
        storeDataInAttributeList(0, 3, positions);
